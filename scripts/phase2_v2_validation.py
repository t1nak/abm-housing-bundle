"""Phase 2 v2 validation: re-run with the wealth-conserving, near-eligible
assortative-help mechanism (Phase 1A revision).
...
Output: outputs/augmented_phase2_v2_results.json
"""
...
def main() -> None:
    ...
    out = OUTPUTS / "augmented_phase2_v2_results.json"
    out.write_text(json.dumps(payload, indent=2))
    print(f"\nSaved {out.relative_to(ROOT)} ({time.time() - t0:.1f}s total)")

    # Consolidated table.
    print("\n" + "=" * 124)
    print("PHASE 2 v2 CONSOLIDATED TABLE (at theta_hat, 10 seeds DE + 10 seeds UK)")
    print("=" * 124)
